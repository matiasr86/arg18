...
def create_c39(height, smallest, text):
    ...
    for mod_path in mod_path_list:
        font_file = mod_path + os.path.sep + "report_aeroo" + os.path.sep + "barcode" + os.path.sep + "FreeMonoBold.ttf"
        if os.path.lexists(font_file):
            font = ImageFont.truetype(font_file, fontsize)

    draw.text(
        (pixel_length / 2 - len(newtext) * (fontsize / 2) / 2 - len(newtext), height + fontsize),
        newtext,
        font=font,
        fill=0,
    )
