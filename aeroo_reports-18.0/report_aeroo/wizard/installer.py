...
class DocsConfigInstaller(models.TransientModel):
    ...
    def _get_image(self):
        ...
        try:
            ...
            path = os.path.join("report_aeroo", "config_pixmaps", "module_banner_1.png")
    ...
    auth_type = fields.Selection([("simple", "Simple Authentication")], "Authentication", default=False)
    ...
    config_logo = fields.Binary(compute="_compute_config_logo", string="Image", default=_get_image)
    # ends Fields
    ...
    def check(self):
        ...
        if self.enabled:
            try:
                ...
                file_data = fp.read()
                docs_client = DOCSConnection(self.host, self.port, username=self.username, password=self.password)
